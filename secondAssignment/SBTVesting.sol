...
contract SBTVesting {
    ...
    constructor(
        ...
        token = SBETERC223(_tokenContractAddress);
    ...
    function calculateVestedTokens(address _beneficiary, uint256 _vestingAmount) public view returns (uint256) {
    ...
    function withdrawInvestorTokens() public {
        uint256 vestedTokens = calculateVestedTokens(msg.sender, INVESTOR_VESTING_AMOUNT);
    ...
    function withdrawAdvisorTokens() public {
        uint256 vestedTokens = calculateVestedTokens(msg.sender, ADVISOR_VESTING_AMOUNT);
        require(vestedTokens > 0, "No tokens are available to withdraw");
    ...
    function withdrawTeamTokens() public {
        uint256 vestedTokens = calculateVestedTokens(msg.sender, TEAM_VESTING_AMOUNT);
        require(vestedTokens > 0, "No tokens are available to withdraw");
